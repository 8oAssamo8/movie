...
if (isset($updateIds)) {
    get_connection();
    $deleteSQL = "update movie set state=1 where id in ($updateIds)";
...
$pageSize = 3;
get_connection();
$selectSQL = "select * from movie where state=2";
$result_set = mysql_query($selectSQL);
close_connection();
if (mysql_num_rows($result_set) == 0) {
    echo "<script type='text/javascript'>
        alert('当前数据库没有未上映电影！请检查之后再进行操作……');
    </script>";
}
...
$nowSQL = "select * from movie where state=2 order by id desc limit $start,$pageSize";
get_connection();
...
<body>
	...
	<br>
	<br>
	<form action="manageUnscreenedMovie.php" id="myform" method="post">
		<table align="center" style="width: 500px;">
			<tr>
				<td colspan="5" align="center" height="60"
					style="border-color: #e5e2e2; line-height: 30px; font-size: 25px;">未上映电影管理</td>
			</tr>
			<tr>
				<td colspan="5">
					<table class="layui-table" border="1" cellspacing="0"
						...
						<tr height="30" style="background-color: #e5e2e2;">
							...
							<td align="center" width="180">电影名（未上映电影）</td>
                        ...
                        <tr height="160">
							...
							<td align="center"><a
								...
								href="manageUnscreenedMovie.php?movieId=<?php echo $movie['id'] ?>"
			...
			<tr>
				<td colspan="5" width="190" height="40" align="center"><input
					...
					value="  上 映 所 选 项  " onclick="submitCheck();"></td>
			</tr>
		</table>
