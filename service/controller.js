...
m.controller('detailsmovie', ['$scope', '$stateParams', '$http', function($scope, $stateParams, $http) {
    ...
    $scope.collect = function() {
        $http({
            method: 'post',
            url: './php/collectmovie.php?action=collect',
            data: $.param($scope.data),
            headers: { 'Content-type': 'application/x-www-form-urlencoded' }
        }).then(function(res) {
            if (res.data == 0) {
                ...
                swal('收藏失败', '请先登录后再进行收藏操作！', 'error');
                history.go(-1);
            } else {
                swal('收藏成功', '', 'success');
                $scope.detial.isCollected = true;
                history.go(-1);
            }
        });
    }
    $scope.unCollect = function() {
        $http({
            method: 'post',
            url: './php/collectmovie.php?action=unCollect',
            data: $.param($scope.data),
            headers: { 'Content-type': 'application/x-www-form-urlencoded' }
        }).then(function(res) {
            if (res.data == 0) {
                ...
                swal('取消收藏失败', '请先登录后再进行收藏操作！', 'error');
                history.go(-1);
            } else {
                swal('取消收藏成功', '', 'success');
                $scope.detial.isCollected = false;
                history.go(-1);
            }
        });
    }
...
//收藏电影
m.controller('collectedMovie', ['$scope', 'collectedmovie', function($scope, collectedmovie) {
    collectedmovie.Collectedmovie().then(function(res) {
        $scope.data = res.data;
        //console.log($scope.data);
        //分页总数
        $scope.pageSize = 12;
