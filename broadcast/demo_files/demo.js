...
window.onload = function () {
    ...
    function removeNode(node) {
        ...
        if(id.substr(0, 1)=="B")
            window.location.href="./deleteBroadcast.php?action=deleteBroadcast&id="+id.substr(1);
        else if(id.substr(0, 1)=="C")
            window.location.href="./deleteBroadcast.php?action=deleteBroadcastComment&id="+id.substr(1);
    }
    ...
    function praiseBox(box, el ) {
        ...
        var newTotal;
        if (txt == '赞') {
            var id = box.children[0].value;
            console.log(id);
            newTotal = oldTotal + 1;
            praisesTotal.setAttribute('total', newTotal);
            window.location.href="./updatePraise.php?action=addNumber&id="+id.substr(1)+"&Number="+newTotal;
            ...
            newTotal = oldTotal - 1;
            praisesTotal.setAttribute('total', newTotal);
            window.location.href="./updatePraise.php?action=reduceNumber&id="+id.substr(1)+"&Number="+newTotal;
    ...
    function reply(box, el) {
        ...
        commentBox.innerHTML =
            ...
            '<p class="comment-text"><span class="user"><div class="user_name">我：</div></span>' + textarea.value + '</p>' +
        ...
        window.location.href="./addBroadcastComment.php?action=addBroadcastComment&id="+id.substr(1)+"&userComment="+textarea.value;
        textarea.value = '';
        textarea.onblur();
    }
    ...
    function praiseReply(el,box) {
        ...
        if (myPraise == 0) {
            ...
            window.location.href="./updatePraise.php?action=addCNumber&id="+id.substr(1)+"&CNumber="+newTotal;
        }
        else {
            var id = box.children[0].value;
            console.log(id);
    ...
    for (var i = 0; i < boxs.length; i++) {
        ...
        boxs[i].onclick = function (e) {
            ...
            switch (el.className) {
                ...
                //赞留言
                case 'comment-praise':
                    praiseReply(el,el.parentNode);
                    break;

                //操作留言
                case 'comment-operate':
                    operate(el,el.parentNode.parentNode);
                    break;
            }
        }
